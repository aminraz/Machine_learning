#+startup: overview
#+property: header-args:python :session *class* :results silent :tangle yes
...
Many of the predictors come in ~int~ format but actually represent categorical data.
...
Making the model using all training dataset:
#+begin_src python
  rf = RandomForestClassifier(n_estimators=100, random_state=42)
...
Upon submitting the submission file, Kaggle calculated the score to be 0.828 ([[https://www.kaggle.com/code/daytatech/categorical-association-with-cramer-s-statistic][kaggle notebook]]). The best score in the leader-board is .840. 
